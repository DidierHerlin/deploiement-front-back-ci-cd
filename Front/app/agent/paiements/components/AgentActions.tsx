  ...
  if (showValidForm) {
    return (
      <div className="bg-blue-50 p-4 border-t border-blue-100">
        <h5 className="font-bold text-blue-900 mb-3 text-sm">Valider le paiement</h5>
        <form onSubmit={handleValiderSubmit} className="space-y-3">
          <div>
            <label className="block text-xs font-semibold text-blue-800 mb-1">Moyen de paiement</label>
            <select 
          ...
          {['MVOLA', 'ORANGE_MONEY', 'AIRTEL_MONEY'].includes(mode) && (
            <div>
              <label className="block text-xs font-semibold text-blue-800 mb-1">Référence de transaction</label>
              <input 
                type="text" 
                className="w-full text-sm p-2 rounded border border-blue-200 outline-none" 
                value={ref} 
